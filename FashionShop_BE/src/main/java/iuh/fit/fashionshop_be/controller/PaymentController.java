...
public class PaymentController {
    ...
    public void handleVnpayReturn(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (status == 1) {
            ...
            out.println("        <p>Cảm ơn bạn đã mua sắm tại Fashion Store. Đơn hàng đang được xử lýyyyy.</p>");
        } else {
        ...
        out.println("        ");
        out.println("        returnBtn.href = deepLink;");
        out.println("        ");
        out.println("        function attemptRedirect() {");
        out.println("            console.log('📱 Attempting to open app...');");
        out.println("            window.location.href = deepLink;"); // Chỉ dùng cách đơn giản nhất
        out.println("        }");
        out.println("        ");
        out.println("        const interval = setInterval(() => {");
        out.println("            count--;");
        out.println("            if(countdownEl) countdownEl.textContent = count;");
        out.println("            ");
